...
class Recommender:
    ...
    def fit(self, matrix):
        ...
        if self.strategy == 'user':
            ...
            user_similarity = 1 - pairwise_distances(normalized_user_ratings, metric='cosine')
            pred = mean_user_ratings + user_similarity.dot(normalized_user_ratings) / np.array([np.abs(user_similarity).
                                                                                               sum(axis=1)]).T
            self.pred = pd.DataFrame(pred.round(2),
            ...
            mean_user_ratings = self.user_item_matrix.mean(axis=1).to_numpy().reshape(-1, 1)
            normalized_user_ratings = self.get_normalized_user_item_matrix(mean_user_ratings)
            item_similarity = 1 - pairwise_distances(normalized_user_ratings.T, metric='cosine')
            pred = mean_user_ratings + normalized_user_ratings.dot(item_similarity) / np.array([np.abs(item_similarity).
                                                                                               sum(axis=1)])
            self.pred = pd.DataFrame(pred.round(2),
                                     index=self.user_item_matrix.index.values,
                                     columns=self.user_item_matrix.columns.values)
    ...
    def recommend_items(self, user_id, k=5):
        ...
        sorted_user_predicted_ratings = user_predicted_ratings_unrated.sort_values(ascending=False, kind='stable')
