...
def RMSE(test_set, cf):
    ...
    test_user_item_mask_matrix = pd.pivot_table(
        test_set, index="UserId", columns="ProductId", values="Rating", aggfunc=lambda _: 1
    )
    ...
    benchmark_ratings = [
        user_mean_ratings[test_case[1]["UserId"]] for test_case in test_set.iterrows()
    ]
...
def precision_at_k(test_set, cf, k):
    ...
    test_user_item_matrix = pd.pivot_table(
        test_set, index="UserId", columns="ProductId", values="Rating", aggfunc="sum"
    )
    ...
    print(
        f"precision@{k} for cf-recommender ({cf.strategy} based): {round(mean_recommender_user_precisions, 5)}"
    )
...
def recall_at_k(test_set, cf, k):
    ...
    for user_id, test_user_ratings in test_user_item_matrix.iterrows():
        ...
        recommender_user_recall = len(recommender_user_relevant_items_recommended) / len(
            test_user_relevant_items
        )
        recommender_user_recalls.append(
            recommender_user_recall
        )  # TODO: export loop to another function

        benchmark_user_relevant_items_recommended = test_user_relevant_items.intersection(
            benchmark_items
        )
        benchmark_user_precision = len(benchmark_user_relevant_items_recommended) / len(
            test_user_relevant_items
        )
    ...
    print(
        f"recall@{k} for cf-recommender ({cf.strategy} based): {round(mean_recommender_recalls, 5)}"
    )
